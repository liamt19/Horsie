...
#define S(Mg, Eg) TaperedScore{(Mg), (Eg)}
		namespace
		{
			constexpr std::array< std::array<TaperedScore, 64>, 6> BonusTables = {
				std::array { // pawns
			...
			const std::array<std::array<TaperedScore, 64>, 12> createPsts() {
				std::array<std::array<TaperedScore, 64>, 12> psts{};

				for (const auto type : { PAWN, HORSIE, BISHOP, ROOK, QUEEN, KING }) {
					const auto value = baseValue(type);
					const auto& bonusTable = BonusTables[static_cast<i32>(type)];

					auto& whitePst = psts[static_cast<i32>(type) * 2 + WHITE];
					auto& blackPst = psts[static_cast<i32>(type) * 2 + BLACK];

					for (i32 square = 0; square < 64; ++square) {
						whitePst[square] =  value + bonusTable[square];
						blackPst[square] = -value - bonusTable[square ^ 56];
					}
				}
