...
namespace Zobrist {
	...
	static ulong EnPassantFileHashes[FILE_NB];
	static ulong BlackHash;
	...
	inline void init() {
		...
		for (int i = 0; i < COLOR_NB; i++) {
			for (int j = 0; j < PIECE_NB; j++) {
				for (int k = 0; k < SQUARE_NB; k++) {
					ColorPieceSquareHashes[i][j][k] = rng();
				}
			}
		}
		...
		{
			EnPassantFileHashes[i] = rng();
		}

	}

	inline void Castle(ulong& hash, CastlingStatus prev, CastlingStatus toRemove) {
	...
	inline void Move(ulong& hash, int from, int to, int color, int pt) {
		hash ^= ColorPieceSquareHashes[color][pt][from] ^ ColorPieceSquareHashes[color][pt][to];
	}

	inline void ToggleSquare(ulong& hash, int color, int pt, int idx) {
		hash ^= ColorPieceSquareHashes[color][pt][idx];
	}

	inline void EnPassant(ulong& hash, int file) {
		hash ^= EnPassantFileHashes[file];
	}

	inline void ChangeToMove(ulong& hash) {
		hash ^= BlackHash;
	}

	inline ulong GetHash(Horsie::Position& position) {
		ulong hash = 0;
