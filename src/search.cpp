...
namespace Horsie {
    ...
    i16 SearchThread::AdjustEval(Position& pos, i32 us, i16 rawEval) const {
        ...
        const auto nonPawnW = History.NonPawnCorrection[us][pos.NonPawnHash(Color::WHITE) % 16384];
        const auto nonPawnB = History.NonPawnCorrection[us][pos.NonPawnHash(Color::BLACK) % 16384];
        const auto corr = (pawn * 50 + nonPawnW * 50 + nonPawnB * 50) / 512;

        return static_cast<i16>(rawEval + corr);
    }


    static void AddToCorrHist(i16& corr, i32 bonus) {
        corr = static_cast<i16>(corr + bonus - corr * std::abs(bonus) / 1024);
    }


    void SearchThread::UpdateCorrectionHistory(Position& pos, i32 diff, i32 depth) {

        const auto scaledBonus = std::clamp((diff * depth) / 8, -256, 256);

        AddToCorrHist(History.PawnCorrection[pos.ToMove][pos.PawnHash() % 16384], scaledBonus);
        AddToCorrHist(History.NonPawnCorrection[pos.ToMove][pos.NonPawnHash(Color::WHITE) % 16384], scaledBonus);
        AddToCorrHist(History.NonPawnCorrection[pos.ToMove][pos.NonPawnHash(Color::BLACK) % 16384], scaledBonus);
    }
