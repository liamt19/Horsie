...
namespace Horsie {
    ...
    i32 SearchThread::Negamax(Position& pos, SearchStackEntry* ss, i32 alpha, i32 beta, i32 depth, bool cutNode) {
        ...
        if (UseNMP
            ...
            ss->MovedPiece = static_cast<u8>(Piece::NONE);
        ...
        if (UseProbcut
            ...
            for (i32 i = 0; i < numCaps; i++) {
                ...
                ss->CurrentMove = m;
                ss->MovedPiece = static_cast<u8>(ourPiece);
    ...
    i16 SearchThread::AdjustEval(Position& pos, SearchStackEntry* ss, i32 us, i16 rawEval) const {
        ...
        const auto npw = History.NonPawnCorrection[us][pos.NonPawnHash(Color::WHITE) % 16384];
        const auto npb = History.NonPawnCorrection[us][pos.NonPawnHash(Color::BLACK) % 16384];
        const auto maj = History.MajorCorrection[us][pos.MajorHash() % 16384];

        i32 cont = 0;
        if (ss->Ply >= 2 && (ss - 2)->MovedPiece != Piece::NONE && (ss - 1)->MovedPiece != Piece::NONE) {
            cont = History.ContCorrection[us][(ss - 2)->MovedPiece][(ss - 2)->CurrentMove.To()][(ss - 1)->MovedPiece][(ss - 1)->CurrentMove.To()];
        }
    ...
    void SearchThread::UpdateCorrectionHistory(Position& pos, SearchStackEntry* ss, i32 diff, i32 depth) {
        ...
        constexpr auto Entries = 16384;
        const auto bonus = std::clamp(diff * depth / 8, -CorrectionEntry::MaxBonus, CorrectionEntry::MaxBonus);
        const auto us = pos.ToMove;

        History.PawnCorrection[us][pos.PawnHash() % Entries].update(bonus);
        History.NonPawnCorrection[us][pos.NonPawnHash(Color::WHITE) % Entries].update(bonus);
        History.NonPawnCorrection[us][pos.NonPawnHash(Color::BLACK) % Entries].update(bonus);
        History.MajorCorrection[us][pos.MajorHash() % Entries].update(bonus);

        if (ss->Ply >= 2 && (ss - 2)->MovedPiece != Piece::NONE && (ss - 1)->MovedPiece != Piece::NONE) {
            History.ContCorrection[us]
                [(ss - 2)->MovedPiece][(ss - 2)->CurrentMove.To()]
                [(ss - 1)->MovedPiece][(ss - 1)->CurrentMove.To()].update(bonus);
        }
    }
